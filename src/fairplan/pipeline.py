...
def normalize(raw_dir: Path, processed_dir: Path, manifest_path: Path | None = None) -> None:
    ...
    if category_rows:
        ...
        # Share of ZIP land area inside CAL FIRE FHSZ High/Very High zones —
        # static table generated by scripts/compute_zip_fire_hazard.py.
        fire_hazard_path = Path("config") / "zip_fire_hazard.csv"
        fire_pct = (
            {r["zip"]: r["fhsz_high_pct"] for r in read_csv(fire_hazard_path)}
